...
function OrderDetails({ order, onClose, onStatus, onShareLocation = () => undefined, onStopLocation = () => undefined, onProof = () => undefined }: { order: Order; onClose: () => void; onStatus: (status: Status) => void; onShareLocation?: (orderNumber: string, coords: { latitude: number; longitude: number }) => void; onStopLocation?: () => void; onProof?: (input: { orderNumber: string; fileName: string; contentType: "image/jpeg" | "image/png" | "image/webp"; data: string }) => void }) {
...
function Dashboard({ dark, onTheme, orders, setOrders, muted, setMuted, menuOpen, setMenuOpen, onNavigate, onActivity, onShareLocation, onStopLocation, onProof }: { dark: boolean; onTheme: () => void; orders: Order[]; setOrders: React.Dispatch<React.SetStateAction<Order[]>>; muted: boolean; setMuted: (value: boolean) => void; menuOpen: boolean; setMenuOpen: (value: boolean) => void; onNavigate: (path: string) => void; onActivity: (value: "available" | "busy" | "unavailable") => void; onShareLocation: (orderNumber: string, coords: { latitude: number; longitude: number }) => void; onStopLocation: () => void; onProof: (input: { orderNumber: string; fileName: string; contentType: "image/jpeg" | "image/png" | "image/webp"; data: string }) => void }) {
  ...
  return <>
    ...
    <main className="page-shell">
      ...
      {selected && <OrderDetails order={selected} onClose={() => setSelected(orders[0])} onStatus={changeStatus} onShareLocation={onShareLocation} onStopLocation={onStopLocation} onProof={onProof} />}
    </main>
  </>;
}
...
function mapLiveOrder(row: any): Order {
  const sourceStatus = row.order.status as string;
  const status: Status = sourceStatus === "in_transit" ? "in_transit" : sourceStatus === "out_for_delivery" ? "out_for_delivery" : sourceStatus === "delivered" ? "delivered" : "ready";
  const updatedAt = row.order.updatedAt ? new Date(row.order.updatedAt) : new Date();
  const date: Range = updatedAt.toDateString() === new Date().toDateString() ? "today" : "yesterday";
  return {
    id: row.order.id, number: row.order.orderNumber, customer: row.order.customerName, phone: row.order.customerPhone, address: row.order.customerAddress, note: row.order.customerNote ?? undefined,
    store: row.store?.name ?? "متجر هسّا", total: row.order.totalCents / 100,
    items: (row.items ?? []).map((item: any) => ({ name: item.productName, quantity: item.quantity, price: item.unitPriceCents / 100 })),
    status, updated: updatedAt.toLocaleString("ar-JO", { dateStyle: "short", timeStyle: "short" }), date, eta: row.deliveryEstimate ? `${row.deliveryEstimate.etaMinutes} دقيقة` : status === "delivered" ? "تم التسليم" : "قيد التحديث",
  };
}
...
export default function Home() {
  ...
  const auth = useAuth();
  const liveOverview = trpc.courier.overview.useQuery(undefined, { enabled: Boolean(auth.user) && auth.user?.role === "staff", retry: false });
  const liveActivity = trpc.courier.activity.useQuery(undefined, { enabled: Boolean(auth.user) && auth.user?.role === "staff", retry: false });
  const statusMutation = trpc.courier.updateStatus.useMutation();
  const locationMutation = trpc.courier.updateLocation.useMutation();
  const stopLocationMutation = trpc.courier.stopLocation.useMutation();
  const proofMutation = trpc.courier.uploadProof.useMutation();
  const activityMutation = trpc.courier.setActivity.useMutation();
  const [dark, setDark] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  const [muted, setMuted] = useState(false);
  const [orders, setOrders] = useState<Order[]>(initialOrders);
  const [activity, setActivity] = useState("unavailable");
  useMemo(() => { if (liveActivity.data?.availability) setActivity(liveActivity.data.availability); return null; }, [liveActivity.data]);
  useMemo(() => { if (liveOverview.data?.orders) setOrders(liveOverview.data.orders.map(mapLiveOrder)); return null; }, [liveOverview.data]);
  ...
  const updateOrder = (status: Status) => {
    ...
    setSelectedOrder(next); setOrders((current) => current.map((order) => order.id === next.id ? next : order));
    if (status === "in_transit" || status === "delivered") statusMutation.mutate({ orderNumber: selectedOrder.number, status }, { onError: (error) => toast.error(error.message) });
  };
  if (auth.loading) return <div className="loading-screen" dir="rtl">جار التحقق من حساب المندوب...</div>;
  if (!auth.user) return <Login />;
  return <div className={`app-frame ${dark ? "theme-dark" : ""}`} dir="rtl">
    {page === "dashboard" && <Dashboard dark={dark} onTheme={() => setDark(!dark)} orders={orders} setOrders={setOrders} muted={muted} setMuted={setMuted} menuOpen={menuOpen} setMenuOpen={setMenuOpen} onNavigate={navigate} onActivity={(value) => activityMutation.mutate({ availability: value }, { onSuccess: () => { setActivity(value); toast.success("تم تحديث حالة نشاطك"); }, onError: (error) => toast.error(error.message) })} onShareLocation={(orderNumber, coords) => locationMutation.mutate({ orderNumber, ...coords }, { onError: (error) => toast.error(error.message) })} onStopLocation={() => stopLocationMutation.mutate()} onProof={(input) => proofMutation.mutate(input, { onError: (error) => toast.error(error.message) })} />}
